...
public partial class Administrator_ManageBookedRoom : System.Web.UI.Page
{
    ...
    {
        TableResult.DataSource = objAdm.PopulateData(currentPage, "GetBookedRoom");
    ...
    protected void TableDelete(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            ...
            Label1.Text = "Are you sure that you want to open room for booking  with ID:" + " " + rec_id.Value + " " + " ?";
    ...
    protected void lbDeleteYes_Click(object sender, EventArgs e)
    {
        try
        {
            if (Administrator() == "1")
            {
                if (objAdm.UpdateRoomBooked(rec_id .Value , "0") == true)
                {
                    DisplaySuccess("Room opened for booking.");
